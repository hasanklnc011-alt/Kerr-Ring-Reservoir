...
NEW = REPO_ROOT / "reports" / "tolerance" / "geometry-sensitivity-averaged.json"
...
def main() -> int:
    ...
    for name in sorted(CROSS_SECTIONS):
        ...
        print(f"R: measuring {name} (offset-averaged) ...", flush=True)
        s = measure(cs, settings, offset_averaged=True)
        row = s.to_dict()
